...
認証・認可、ユーザトラッキング、ショッピングカートのようなクライアントによる情報保持など、異なるユースケースで用いられます。

セッションは典型的な方法としては`Cookie`を使って実装されていますが、
ヘッダーを使うことでも実現できます。
...
* *サーバのどこにペイロードが保管されているか:* インメモリかフォルダかredisか...
...
これらの関数はオプショナルで、設定のためのlambda関数を受け取ります。
